...
void Application::CheckNewVersion(Ota& ota) {
    ...
    CustomOta custom_ota = GetCustomOta();
    custom_ota.CheckVersion();
#endif
...
#ifdef CONFIG_USE_CUSTOM_OTA
        if (ota.HasNewVersion() || (custom_ota.HasNewVersion() && custom_ota.forced())) {
            Ota *_ota = nullptr;
...
void Application::CheckNewVersionForCustom() {
    ...
    if (custom_ota.HasNewVersion()) {
        ...
        std::string message = std::string(Lang::Strings::NEW_VERSION) + custom_ota.GetFirmwareVersion();
        ...
        bool upgrade_success = custom_ota.StartUpgrade([display](int progress, size_t speed) {
            char buffer[64];
            snprintf(buffer, sizeof(buffer), "%d%% %uKB/s", progress, speed / 1024);
            display->SetChatMessage("system", buffer);
        });
        ...
        hint += ": ";
        hint += custom_ota.GetCurrentVersion();
        hint += "\n";
...
void Application::Start() {
    ...
    // xTaskCreate([](void *arg)
    //             {
    //     vTaskDelay(pdMS_TO_TICKS(200));  // Wait for LVGL task to stabilize
    //     ui_init();
    //     vTaskDelete(NULL); }, "ui_init", 4096, nullptr, 1, nullptr);
    ...
    // Initialize alarm service (after network is ready for time sync)
    alarm_service_init();
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                Schedule([this]() {
                    ...
                    if (device_state_ == kDeviceStateIdle || device_state_ == kDeviceStateListening) {
                        ...
                        audio_service_.PushMuteAudio(CONFIG_SPEAKING_FIRST_MUTE_TIME);
#endif
                    }
                });
...
void Application::DeInitAudioService() {
    audio_service_.DeInitialize();
}

void Application::InitAudioService() {
    audio_service_.DeInitialize();
}

bool Application::AudioServiceIsInit() {
    return audio_service_.IsInitialize();
}
