...
#define CONFIG_OPUS_CODEC_TASK_STACK_SIZE (4096 * 7)
#endif

#ifndef CONFIG_AUDIO_INPUT_TASK_STACK_SIZE
#define CONFIG_AUDIO_INPUT_TASK_STACK_SIZE (2048 * 3)
#endif

#ifndef CONFIG_AUDIO_OUTPUT_TASK_STACK_SIZE
#define CONFIG_AUDIO_OUTPUT_TASK_STACK_SIZE (4096)
#endif
...
void AudioService::DeInitialize()
{
    Stop();
...
void AudioService::Start()
{
    ...
    xTaskCreatePinnedToCore([](void *arg)
        ...
        vTaskDelete(NULL); }, "audio_input", CONFIG_AUDIO_INPUT_TASK_STACK_SIZE, this, 8, &audio_input_task_handle_, 1);
#else
    /* Start the audio input task */
    xTaskCreate([](void *arg)
        ...
        AudioService* audio_service = (AudioService*)arg;
        audio_service->AudioInputTask();
        vTaskDelete(NULL); }, "audio_input", CONFIG_AUDIO_INPUT_TASK_STACK_SIZE, this, 8, &audio_input_task_handle_);
#endif
    ...
    xTaskCreate([](void *arg)
        ...
        vTaskDelete(NULL); }, "audio_output", CONFIG_AUDIO_OUTPUT_TASK_STACK_SIZE, this, 3, &audio_output_task_handle_);
    ...
    xTaskCreate([](void *arg)
        ...
        vTaskDelete(NULL); }, "opus_codec", CONFIG_OPUS_CODEC_TASK_STACK_SIZE, this, 2, &opus_codec_task_handle_);
}
...
void AudioService::AudioOutputTask()
{
    while (true)
    {
        ...
        if (curr_play_sound_ != nullptr && audio_playback_queue_.empty() && audio_decode_queue_.empty())
        {
            curr_play_sound_ = nullptr;
        }
    }
...
void AudioService::OpusCodecTask()
{
    while (true)
    {
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE)
        {
            ...
            if (free_sram < 10000)
            {
                ESP_LOGE(TAG, "Opus codec task is out of memory, free_sram: %d", free_sram);
                break;
            }
...
void AudioService::PlaySound(const std::string_view &ogg)
{
    ...
    while (true)
    {
        ...
        while (seg_idx < page_segments)
        {
            ...
            auto task = std::make_unique<AudioTask>();
            task->type = kAudioTaskTypeDecodeToPlaybackQueue;
            task->oups.resize(payload_size);
            memcpy(task->oups.data(), p3->payload, payload_size);
            {
                lock.lock();
                audio_playback_queue_.push_back(std::move(task));
                audio_queue_cv_.notify_all();
...
void AudioService::PushMuteAudio(int mute_time)
{
    for (size_t i = 0; i < (mute_time / 60); i++)
    {
        auto packet = GetMutePacket();
#ifdef CONFIG_USE_AUDIO_CODEC_DECODE_OPUS
        auto task = std::make_unique<AudioTask>();
        task->type = kAudioTaskTypeDecodeToPlaybackQueue;
        task->oups.resize(packet->payload.size());
        memcpy(task->oups.data(), packet->payload.data(), packet->payload.size());
        {
            lock.lock();
            audio_playback_queue_.push_back(std::move(task));
            audio_queue_cv_.notify_all();
