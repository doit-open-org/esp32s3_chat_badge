                             ...
                             int32_t event_id, void *event_data)
{
    ...
    case IP_EVENT_STA_GOT_IP:
    {
        ...
        if (ble_is_connected == true)
        {
            ...
            // 在发送WiFi连接报告之前调用回调，让上层有机会发送激活码
            if (g_cbs.before_wifi_report_cb)
            {
                g_cbs.before_wifi_report_cb(g_cbs_arg);
            }
...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    case ESP_BLUFI_EVENT_RECV_STA_PASSWD:
        ...
        esp_wifi_set_config(WIFI_IF_STA, &sta_config);
        BLUFI_INFO("Received STA password (%u bytes)\n", (unsigned)param->sta_passwd.passwd_len);
        break;
    ...
    case ESP_BLUFI_EVENT_RECV_SOFTAP_PASSWD:
        ...
        esp_wifi_set_config(WIFI_IF_AP, &ap_config);
        BLUFI_INFO("Received SoftAP password (%u bytes)\n", (unsigned)param->softap_passwd.passwd_len);
        break;
