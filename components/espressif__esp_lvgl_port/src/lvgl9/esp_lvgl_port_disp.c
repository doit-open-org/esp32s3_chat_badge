...
typedef struct {
    ...
    bool                      enable_tearing_effect; /* 是否开启撕裂处理 */
    SemaphoreHandle_t         tearing_semaphore; /* 撕裂处理需要的信号量 */
    uint32_t                  tearing_timeout_ms; /* 撕裂信号的超时时间 */
#if LVGL_PORT_PPA
...
IRAM_ATTR static void te_gpio_isr_handler(void *arg) {
  lvgl_port_display_ctx_t* disp_ctx = (lvgl_port_display_ctx_t*)arg;
  BaseType_t higher = pdFALSE;
  xSemaphoreGiveFromISR(disp_ctx->tearing_semaphore, &higher);
  if(higher == pdTRUE) {
    portYIELD_FROM_ISR();
  }
}
...
static lv_display_t *lvgl_port_add_disp_priv(const lvgl_port_display_cfg_t *disp_cfg, const lvgl_port_disp_priv_cfg_t *priv_cfg)
{
  ...
  /* 注册te引脚中断 */
  if (disp_cfg->enable_tearing_effect) {
    gpio_install_isr_service(0);
    gpio_set_intr_type(disp_cfg->te_gpio, disp_cfg->tear_effect_level_edge == 1
                                              ? GPIO_INTR_POSEDGE
                                              : GPIO_INTR_NEGEDGE);
    gpio_set_pull_mode(disp_cfg->te_gpio, GPIO_PULLUP_ENABLE);
    gpio_set_direction(disp_cfg->te_gpio, GPIO_MODE_INPUT);
    SemaphoreHandle_t tearing_semaphore = xSemaphoreCreateCounting(1, 0);
    // 将撕裂处理配置项放到disp_ctx中
    disp_ctx->enable_tearing_effect = disp_cfg->enable_tearing_effect;
    disp_ctx->tearing_semaphore = tearing_semaphore;
    disp_ctx->tearing_timeout_ms = disp_cfg->tearing_timeout_ms;
    // 注册中断函数
    gpio_isr_handler_add(disp_cfg->te_gpio, te_gpio_isr_handler, disp_ctx);
  }
...
static void lvgl_port_flush_callback(lv_display_t *drv, const lv_area_t *area, uint8_t *color_map)
{
    ...
    if ((disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_RGB || disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_DSI) && (disp_ctx->flags.direct_mode || disp_ctx->flags.full_refresh)) {
      ...
      if (disp_ctx->enable_tearing_effect) {
        // 撕裂处理
        static int cnt = 0;
        cnt++;
        if (cnt == 1) {
          xSemaphoreTake(disp_ctx->tearing_semaphore, 0);
          if (xSemaphoreTake(disp_ctx->tearing_semaphore,
                             pdMS_TO_TICKS(disp_ctx->tearing_timeout_ms)) ==
              pdFALSE) {
            ESP_LOGW(TAG, "TE timeout");
          }
        }
        esp_lcd_panel_draw_bitmap(disp_ctx->panel_handle, offsetx1, offsety1,
